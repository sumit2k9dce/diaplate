...
function detectIntent(message) {
  ...
  if (/^(?:hi|hello|hey|start)$/i.test(text)) return "WELCOME"; if (/^help$/i.test(text)) return "HELP";
...
if (intent === "HELP") {
    return `📖 *DiaPlate — All Commands*

📸 Send a *meal photo* → T-Plate analysis + calories
🔬 Send a *lab report photo* → explain your blood test
...
❓ Ask anything → "Can I eat biryani?"

📊 *summary* → weekly health report
🔬 *hba1c* → estimated HbA1c
📈 *correlation* → food-sugar patterns
📋 *share doctor* → doctor-ready report
📊 *my data* → your tracking stats
⚙️ *menu* → settings & controls
